 ...
 *           good test.
 * @param includeCostars true iff costars should be printed in addition
 *                       to movie credits.
 */
static void publishFindings(const imdb& db, const string& player, bool includeCostars) {
  vector<film> credits;
  ...
  if (includeCostars) listCostars(player, credits, db);
}

static bool commandProperlyFormatted(int argc, const char *argv[]) {
  if (argc != 2 && argc != 3) return false;
  if (argc == 3) return strcasecmp(argv[1], "--just-movies") == 0;
  return true;
}

int main(int argc, const char *argv[]) {
  if (!commandProperlyFormatted(argc, argv)) {
    cerr << "Usage: " << argv[0] << " [--just-movies] <actor>" << endl;
  ...
  bool includeCostars = argc == 2;
  string player = argv[argc - 1];
  publishFindings(db, player, includeCostars);
  return 0;
}
